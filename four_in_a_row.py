...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        # list of tuples containing columns, and board_copy (for all valid moves).
        moves = get_child_boards(player, board)

        # set the score to the minimum and set best_move to the default first move.
        score = -math.inf
        best_move = moves[0]
        ...
        for move in moves:
            ...
            # recursive call value as the next player, if the new score is the maximum, then record the current maximum action as the current best
            max_score = max(score, value(next_player, board_copy, depth_limit - 1)["score"])
            if max_score > score:
                best_move = column
                score = max_score

        # all move paths of the largest player are traversed, returning their best move and score
        return {"column": best_move, "score": score}
    
    def min_value(player, board, depth_limit):
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        # list of tuples containing columns, and board_copy (for all valid actions).
        moves = get_child_boards(player, board)
        # set the score to the maximum and set best_move to the default first move
        score = math.inf
        best_move = moves[0]
        # traverse all move paths
        for move in moves:
            ...
            # recursive call value as the next player, if the new score is the minimum, record the current column as the current best minimum action
            min_score = min(score, value(next_player, board_copy, depth_limit - 1)["score"])
            if min_score < score:
                best_move = column
                score = min_score

        # all move paths of the smallest player are traversed, returning his best move and score
        return {"column": best_move, "score": score}
